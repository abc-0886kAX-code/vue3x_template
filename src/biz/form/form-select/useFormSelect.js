...
export function useFormSelect(sign) {
  ...
  const schema = computed(() => {
    return mergeObject({
      ...define,
      ...mergeObject({
        ...selectSchema.schema.value,
        options: handleSelectOptions(selectSchema.schema.value.uselabel, selectSchema.schema.value.usevalue, selectSchema.schema.value.options),
      }),
    })
  })
